...
def main():
    ...
    parser_generate.add_argument(
        '--validate-roles',
        action='store_true',
        help='If set, run an optional LLM-based validation of placeholder roles (may increase latency and API usage).'
    )
    parser_generate.add_argument(
        '--output-path',
    ...
    if args.command == 'generate':
        ...
        # Honor CLI toggle for role validation
        if getattr(args, 'validate_roles', False):
            slide_generator.use_llm_role_validation = True
