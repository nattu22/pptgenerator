...
class ExecutionOrchestrator:
    ...
    def _fill_content(self, placeholder, ph_id: int, ph_info: Dict,
        ...
        if self._check_text_overflow(bullets, ph_info['area']):
    ...
    def _check_text_overflow(self, bullets: List[str], area: float) -> bool:
        """FIX #4: Validate if text fits in placeholder area"""
        # Simple heuristic: avg chars per sq inch
        # 18pt font ~ 0.25 inch height. 1 line ~ 0.35 inch with spacing.
        # 10 words per line ~ 60 chars.
        # 60 chars / 0.35 inch height * 1 inch width ? No.

        # 1 sq inch can hold approx 80-100 characters at 12pt, maybe 40-50 at 18pt.

        total_chars = sum(len(b) for b in bullets)

        # Allow 50 chars per sq inch (generous but safe for 18pt)
        capacity = area * 50

        if total_chars > capacity:
            logger.info(f"      Overflow check: {total_chars} chars > {capacity:.0f} capacity (area {area:.1f})")
            return True
        return False
